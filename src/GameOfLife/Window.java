...
public class Window implements Runnable {
    ...
    /*
    метод отвечающий за отрисовку окна
     */
    void initFrame() {
        frame = new JFrame();
        ...
        frame.setLocationRelativeTo(null);//размещает окно по центру экрана
    ...
    void initBoxes() {
        ...
        //выбираем конкретную клетку на экране
        for (int x = 0; x < Config.WIDTH; x ++)
            for (int y = 0; y < Config.HEIGHT; y ++)
                //проверяем 8 клеток вокруг,центральную исключаем
                for (int sx = -1; sx <= +1; sx ++)
                    for (int sy = -1; sy <= +1; sy ++)
                        if (!(sx == 0 && sy == 0))
                            //добавляем каждую клетку в качестве соседа
